...
def admin_media_post():
	'''This method provides you with the functionality to create new media. It will
...
@app.route('/admin/server/', methods=['POST'])
def admin_server_post():
	'''This method provides you with the functionality to set server data. It
	will create a new dataset or fail if one with the given identifier/format
	already exists.
	...
	**URI PATTERN**: You can use the following placeholders in the URI pattern:
	...
	# _Only_ admins are allowed to add new server.
	try:
	...
	try:
		affected_rows = cur.executemany('''insert into lf_server 
			(id, format, uri_pattern) values (%s, %s, %s) ''', sqldata )
...
def admin_subject_post():
	'''This method provides you with the functionality to set subject data. It
	will create a new dataset or fail if one with the given identifier already
	exists.
	...
	try:
		affected_rows = cur.executemany('''insert into lf_subject
			(id, language, name) values (%s, %s, %s) ''', sqldata )
	except IntegrityError as e:
		return str(e), 409
	db.commit()
...
@app.route('/admin/file/', methods=['POST'])
def admin_file_post():
...
def admin_organization_post():
	...
	try:
		if not get_authorization( request.authorization ).is_editor():
			return 'Only admins and editors are allowed to create/modify organization', 401
	except KeyError as e:
		return str(e), 401
	...
	try:
		affected_rows = cur.executemany('''insert into lf_organization
			(id, name, vcard_uri, parent_organization)
			values (%s, %s, %s, %s) ''', sqldata )
	except IntegrityError as e:
		return str(e), 409
	db.commit()
...
def admin_user_post():
	...
	**Username generator**:
	...
	try:
		if not get_authorization( request.authorization ).is_admin():
			return 'Only administrators can create new user', 401
	except KeyError as e:
		return str(e), 401
...
def admin_user_passwd_post():
	...
	try:
		if not get_authorization( request.authorization ).is_admin():
			return 'Only an administrator can set an initial password.', 401
	except KeyError as e:
		return str(e), 401
	...
	try:
		cur.executemany('''update lf_user
				set salt=%s, passwd=%s
				where isnull(passwd) and  id=%s''', sqldata_ready )
...
def admin_access_post():
	...
	try:
		if not get_authorization( request.authorization ).is_admin():
			return 'Only admins are allowed to create access data', 401
	except KeyError as e:
		return str(e), 401
...
def admin_user_group_post():
	...
	affected_rows = 0
	try:
		affected_rows = cur.executemany('''insert into lf_user_group
...
@app.route('/admin/user/organization/', methods=['POST'])
def admin_user_organization_post():
	...
	affected_rows = 0
	try:
		affected_rows = cur.executemany('''insert into 
...
@app.route('/admin/media/subject/', methods=['POST'])
def admin_media_subject_post():
...
@app.route('/admin/series/subject/', methods=['POST'])
def admin_series_subject_post():
