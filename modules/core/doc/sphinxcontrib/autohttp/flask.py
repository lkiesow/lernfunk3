...
class AutoflaskDirective(Directive):
    ...
    def within_path(self):
        try:
            path = re.split(r'\s*,\s*', self.options['within-path'])
        except KeyError:
            return frozenset()
    ...
    def without_path(self):
        try:
            path = re.split(r'\s*,\s*', self.options['without-path'])
        except KeyError:
            return frozenset()
        return frozenset(path)

    @property
    def only_methods(self):
        try:
            methods = re.split(r'\s*,\s*', self.options['only-methods'])
        except KeyError:
            return frozenset()
        return frozenset(methods)

    @property
    def undoc_methods(self):
        try:
            methods = re.split(r'\s*,\s*', self.options['undoc-methods'])
        except KeyError:
            return frozenset()
        return frozenset(methods)
    ...
    def make_rst(self):
        ...
        for method, path, endpoint in get_routes(app):
            ...
            if endpoint in self.undoc_endpoints:
                continue
            if method in self.undoc_methods:
                continue
            if not True in ([ path.startswith(p) for p in self.within_path ] or [True]):
                continue
            if self.only_methods and not method in self.only_methods:
                continue
            if True in [ path.startswith(p) for p in self.without_path ]:
                continue
            try:
